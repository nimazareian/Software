    ...
    const Update& event, boost::sml::back::process<MoveFSM::Update> processEvent)
{
    ...
    double robot_obstacle_inflation_factor =
        strategy->getAiConfig().robot_navigation_obstacle_config().robot_obstacle_inflation_factor() + 0.5;
    ...
    auto goal_post_offset_vector =
        Vector(0, strategy->getAiConfig().crease_defender_config().goal_post_offset_chipping());
    ...
    else if (nearest_enemy.has_value())
    {
        ...
        bool ball_is_near_friendly =
            ball_distance < nearest_enemy_distance *
                                strategy->getAiConfig().crease_defender_config().max_get_ball_ratio_threshold();
        bool ball_is_within_max_range =
            ball_distance <= strategy->getAiConfig().crease_defender_config().max_get_ball_radius_m();
        bool ball_is_slow = event.common.world_ptr->ball().velocity().length() <=
                            strategy->getAiConfig().crease_defender_config().max_ball_speed_to_get_m_per_s();
