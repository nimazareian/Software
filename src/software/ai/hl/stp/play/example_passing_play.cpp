...
void ExamplePassingPlay::getNextTactics(TacticCoroutine::push_type &yield, const World &world)
{
    ...
    std::shared_ptr<StopTactic> stop_tactic = std::make_shared<StopTactic>();
    int ticker = 0;
    ...
    do
    {
        do
        {
            ...
            Pass pass = Pass(Point(-0.8, -1), Point(-0.7, 0.8), ai_config.passing_config().max_pass_speed_m_per_s());
            ...
            if (!ticker)
            {
                LOG(INFO) << "[EXAMPLE PASSING PLAY] chosen pass has speed: " << pass.speed();
            }
            if (attacker_tactic->done())
            {
                result = { receiver_tactic, stop_tactic };
            }
            yield({result});
