...
FreeKickPlayFSM::FreeKickPlayFSM(std::shared_ptr<Strategy> strategy)
    : strategy(strategy),
      ...
      shoot_tactic(std::make_shared<KickSkill::SkillTactic>(strategy)),
      chip_tactic(std::make_shared<ChipSkill::SkillTactic>(strategy)),
      passer_tactic(std::make_shared<KickSkill::SkillTactic>(strategy)),
...
void FreeKickPlayFSM::setupPosition(const Update &event)
{
    ...
    setReceiverAndDefenderTactics(tactics_to_run, event, num_receivers, num_defenders);
...
void FreeKickPlayFSM::chipBall(const Update &event)
{
    ...
    Vector chip_vector = chip_target - event.common.world_ptr->ball().position();

    chip_tactic->updateControlParams({event.common.world_ptr->ball().position(),
                                      chip_vector.orientation(), chip_vector.length()});
