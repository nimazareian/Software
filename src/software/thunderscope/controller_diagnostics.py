...
class ControllerDiagnostics(object):
    ...
    def __init__(self, proto_unix_io):
        ...
        self.controller.button_x.when_pressed = self.__on_button_x_pressed
        self.controller.button_y.when_pressed = self.__on_button_y_pressed
    ...
    def __on_button_x_pressed(self, button):
        if time.time() - self.last_packet_sent < 0.01:
            return
        ...
        power_control = PowerControl()
        power_control.geneva_slot = 1
        power_control.chicker.auto_chip_or_kick.autochip_distance_meters = AUTO_CHIP_SPEED_MS
        self.proto_unix_io.send_proto(PowerControl, power_control)
        self.controller.set_rumble(0.3, 0.3, AUTO_KICK_OFF_RUMBLE_LENGTH_MS)
        self.last_packet_sent = time.time()

    def __on_button_y_pressed(self, button):
        if time.time() - self.last_packet_sent < 0.01:
            return
